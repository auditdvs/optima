...
export default function AssignmentLetterList({ refreshTrigger }: AssignmentLetterListProps) {
  ...
  const [letterToPrint, setLetterToPrint] = useState<AssignmentLetter | null>(null);
  ...
  const formatAuditPeriod = (letter: AssignmentLetter) => {
    ...
    if (letter.audit_period_start && letter.audit_period_end) {
      return `${formatMonthYear(letter.audit_period_start)} s.d. ${formatMonthYear(letter.audit_period_end)}`;
    }
    if (letter.audit_start_date && letter.audit_end_date) {
      return `${formatMonthYear(letter.audit_start_date)} s.d. ${formatMonthYear(letter.audit_end_date)}`;
    }
    return '-';
  };

  const formatAuditExecution = (letter: AssignmentLetter) => {
    const monthNames = [
      'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni',
      'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'
    ];
    ...
    if (letter.audit_start_date && letter.audit_end_date) {
      return `${formatFullDate(letter.audit_start_date)} s.d. ${formatFullDate(letter.audit_end_date)}`;
    }
    return '-';
  };
  ...
  const handlePrint = async (letter: AssignmentLetter) => {
    ...
    try {
      ...
      toast.dismiss();
      pdf.save(`Surat_Tugas_${letter.assigment_letter}.pdf`);
  ...
  const handleDownloadLPJ = async (letterId: string) => {
    try {
      ...
      const downloadFileName = `LPJ_${data.branch_name?.replace(/[^a-zA-Z0-9]/g, '_')}_${data.assigment_letter?.replace(/[^a-zA-Z0-9]/g, '_')}.${extension}`;
            ...
            <div className="mt-3">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Pelaksanaan Audit</label>
                    <p className="text-sm text-gray-900">{formatAuditExecution(selectedLetter)}</p>
                  </div>
                  <div>
        ...
        <div style={{ position: 'absolute', left: '-9999px', top: 0 }}>
          <div ref={printRef}>
            <AssignmentLetterPrint 
              data={letterToPrint} 
              type={
                letterToPrint.audit_type?.toLowerCase().includes('khusus') 
                  ? 'khusus' 
                  : 'regular'
              }
            />
