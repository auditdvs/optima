...
const THC = () => {
  ...
  const [thcError, setThcError] = useState(false);
  ...
  const fetchTHCRequests = async () => {
    ...
    try {
      ...
      const { data: thcData, error } = await query.order('created_at', { ascending: false });
      
      if (error) {
        console.error('Error fetching THC data:', error);
        throw error;
      }

      // If we have data, fetch the profile names separately
      if (thcData && thcData.length > 0) {
        // Get unique user IDs
        const userIds = [...new Set(thcData.map(req => req.requested_by))];
        ...
        if (profilesError) {
          console.error('Error fetching profiles for THC:', profilesError);
        }
        ...
        // Combine the data
        const combinedData = thcData.map(request => ({
          ...request,
      ...
      console.error('Error fetching THC data:', error);
      toast.error('Failed to load THC data');
      setThcError(true);
    } finally {
      setLoadingTHC(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('THC request submitted successfully');
      setFormData({ branchId: '', startDate: '', endDate: '' });
      setShowForm(false);
      fetchTHCRequests(); // Refresh the list
    } catch (error) {
      console.error('Error submitting THC request:', error);
      toast.error('Failed to submit THC request');
    }
  };
  ...
  const handleAdminResponse = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error updating THC request:', error);
      toast.error('Failed to update THC request');
    }
  };

  // Handler untuk delete THC request
  const handleDeleteRequest = async (request: any) => {
    try {
      console.log("Starting delete for THC request:", request.id);
      ...
      if (request.result_path) {
        try {
          ...
          try {
            ...
            if (urlObj.pathname.includes('/thc/')) {
              const bucketPart = '/thc/';
          ...
          if (!filePath) {
            const parts = request.result_path.split('/thc/');
            if (parts.length > 1) {
      ...
      console.log("Deleting database record for THC request:", request.id);
      const { error } = await supabase
        .from('thc')
        .delete()
        .eq('id', request.id);
      ...
      toast.success('THC request deleted successfully');
      
      // Update UI immediately while waiting for refresh
      setRequests((current) => current.filter(item => item.id !== request.id));
      
      // Refresh data to ensure consistency
      await fetchTHCRequests();
    } catch (err) {
      console.error('Error deleting THC request:', err);
      toast.error('Failed to delete THC request');
    }
  };
    ...
    <div className="w-full h-full flex flex-col">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl font-bold">THC</h1>
            <p className="text-m text-gray-500 mt-1">
              Request THC data berdasarkan <strong>kode cabang</strong> dan <strong>periode transaksi</strong>. Mohon dibaca baik-baik, ini hanya mengambil data transaksi harian saja bukan <strong>detail pelunasan, anggota keluar ataupun kas dalam perjalanan</strong>.
            </p>
          </div>
          <button
            onClick={() => {
            ...
            disabled={loadingTHC}
          >
            <RefreshCw size={18} className={`text-gray-500 ${loadingTHC ? 'animate-spin' : ''}`} />
          </button>
        </div>
      ...
      {loadingTHC ? (
        ...
        <div className="flex flex-col items-center justify-center p-8 bg-rose-50 border border-rose-200 rounded-lg text-center">
          ...
          <p className="text-rose-700 mt-1">Terjadi kesalahan saat mengambil data THC.</p>
          <button 
            onClick={() => {
              setThcError(false);
              setLoadingTHC(true);
                      ...
                      <div className="flex gap-2">
                        {request.result_path && (
                          <button
                            ...
                            title="Download THC data"
                            onClick={() => handleCustomDownload(request.result_path, `THC-${request.branch_id}-${request.start_date}-${request.end_date}.csv`)}
                          >
                        ...
                        {userRole === 'superadmin' && (
                          <button
                            onClick={() => {
                              ...
                              if (window.confirm(`Are you sure you want to delete this THC request for branch ${request.branch_id}?`)) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                <label className="block text-gray-700 mb-2">Branch ID</label>
                <input
                  type="text"
      ...
      {isAdmin && selectedRequest && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-xl font-semibold mb-4">Respond to THC Request</h2>
