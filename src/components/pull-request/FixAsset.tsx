...
const FixAsset = () => {
  ...
  const fetchAssetRequests = async () => {
    ...
    try {
      ...
      const { data: assetData, error } = await query.order('created_at', { ascending: false });
      
      if (error) throw error;

      // If we have data, fetch profile data separately
      if (assetData && assetData.length > 0) {
        // Get unique user IDs
        const userIds = [...new Set(assetData.map(req => req.requested_by))];
        ...
        if (profilesError) {
          console.error('Error fetching profiles for Fix Asset:', profilesError);
          // Continue with asset data even if profiles fail
        }
        ...
        // Combine data
        const combinedData = assetData.map(request => ({
          ...request,
      ...
      console.error('Error fetching Fix Asset requests:', error);
      toast.error('Gagal mengambil data Fix Asset requests');
      setAssetsError(true);
    } finally {
  ...
  // Submit handler for Fix Asset request
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    
    // Format branch code to ensure 3 digits (add leading zeros)
    const formattedBranchCode = formData.branchCode.padStart(3, '0');
    ...
    try {
      ...
      toast.success('Fix Asset request berhasil dikirim');
      setFormData({ branchCode: '' });
      setShowForm(false);
      fetchAssetRequests();
    } catch (error) {
      console.error('Error submitting Fix Asset request:', error);
      toast.error('Gagal mengirim permintaan Fix Asset');
    }
  };
  ...
  const handleDeleteRequest = async (requestId: string) => {
    try {
      setLoading(true);
      console.log("Starting delete for Fix Asset request:", requestId);
      
      console.log("Deleting database record for Fix Asset request:", requestId);
      ...
      toast.success('Fix Asset request deleted successfully');
      
      await fetchAssetRequests();
    } catch (error) {
      console.error('Error deleting Fix Asset request:', error);
      toast.error('Failed to delete Fix Asset request');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="w-full h-full flex flex-col">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          <div>
            <h1 className="text-2xl font-bold">Fix Asset Requests</h1>
            <p className="text-sm text-gray-500 mt-1">
              Request data fix asset berdasarkan kode cabang.
            </p>
          </div>
        </div>
        ...
        <div>
          <button
            ...
            className="px-4 py-2 rounded transition-colors bg-emerald-600 text-white hover:bg-emerald-700"
            title="Request Fix Asset Data"
          >
            Request Data
          </button>
        </div>
      ...
      <div className="mb-4 flex flex-wrap gap-4 items-center">
        ...
        <div className="flex items-center gap-2">
          <span className="px-2 py-1 rounded-full bg-blue-100 text-blue-800 text-xs font-semibold">running</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-600">: Selesai, siap diunduh</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="px-2 py-1 rounded-full bg-rose-100 text-rose-800 text-xs font-semibold">error</span>
          <span className="text-sm text-gray-600">: Gagal diproses</span>
        </div>
      </div>

      {/* Fix Asset Requests Table */}
      {loadingAssets ? (
        ...
        <div className="flex flex-col items-center justify-center p-8 bg-rose-50 border border-rose-200 rounded-lg text-center">
          ...
          <p className="text-rose-700 mt-1">Terjadi kesalahan saat mengambil data Fix Asset requests.</p>
          <button 
            onClick={() => {
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex gap-2">
                        ...
                        {userRole === 'superadmin' && (
                          <button
                            onClick={() => {
                              if (window.confirm(`Are you sure you want to delete this Fix Asset request for branch ${request.branch_code}?`)) {
                                handleDeleteRequest(request.id);
                              }
                            }}
                            className="text-rose-600 hover:text-rose-800 p-1.5 rounded-full hover:bg-rose-50"
                            title="Delete Fix Asset request"
                          >
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-lg p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-xl font-semibold mb-4">Request Fix Asset</h2>
