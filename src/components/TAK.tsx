...
const TAK = () => {
  ...
  // Check if TAK requests are allowed at current time
  ...
  const fetchTAKRequests = async () => {
    ...
    try {
      ...
      const { data: takData, error } = await query.order('created_at', { ascending: false });
      ...
      if (takData && takData.length > 0) {
        // Get unique user IDs
        const userIds = [...new Set(takData.map(req => req.requested_by))];
        ...
        if (profilesError) {
          console.error('Error fetching profiles for TAK:', profilesError);
        }
        ...
        // Combine the data
        const combinedData = takData.map(request => ({
          ...request,
      ...
      console.error('Error fetching TAK data:', error);
      toast.error('Failed to load TAK data');
      setTakError(true);
    } finally {
      setLoadingTAK(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isRequestAllowed()) {
      ...
      toast.error(`TAK requests hanya tersedia pada jam 18:00-05:00 (hari kerja) dan 24/7 (weekend). Silakan coba lagi pada jam ${timeStr}.`);
      return;
    }
    ...
    try {
      ...
      toast.success('TAK request submitted successfully');
      setFormData({ branchId: '', startDate: '', endDate: '' });
      setShowForm(false);
      fetchTAKRequests(); // Refresh the list
    } catch (error) {
      console.error('Error submitting TAK request:', error);
      toast.error('Failed to submit TAK request');
    }
  };
  ...
  const handleAdminResponse = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error updating TAK request:', error);
      toast.error('Failed to update TAK request');
    }
  };

  // Handler untuk delete TAK request
  const handleDeleteRequest = async (request: any) => {
    try {
      console.log("Starting delete for TAK request:", request.id);
      ...
      if (request.result_path) {
        try {
          ...
          try {
            ...
            if (urlObj.pathname.includes('/tak/')) {
              const bucketPart = '/tak/';
          ...
          if (!filePath) {
            const parts = request.result_path.split('/tak/');
            if (parts.length > 1) {
      ...
      console.log("Deleting database record for TAK request:", request.id);
      const { error } = await supabase
        .from('tak')
        .delete()
        .eq('id', request.id);
      ...
      toast.success('TAK request deleted successfully');
      
      // Update UI immediately while waiting for refresh
      setRequests((current) => current.filter(item => item.id !== request.id));
      
      // Refresh data to ensure consistency
      await fetchTAKRequests();
    } catch (err) {
      console.error('Error deleting TAK request:', err);
      toast.error('Failed to delete TAK request');
    }
  };
    ...
    <div className="w-full h-full flex flex-col">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl font-bold">TAK</h1>
            <p className="text-m text-gray-500 mt-1">
              Request TAK data berdasarkan <strong>kode cabang</strong> dan <strong>periode transaksi</strong>. Mohon dibaca baik-baik, ini mengambil data TAK (Titipan Anggota Koperasi).
            </p>
          </div>
          <button
            onClick={() => {
            ...
            disabled={loadingTAK}
          >
            <RefreshCw size={18} className={`text-gray-500 ${loadingTAK ? 'animate-spin' : ''}`} />
          </button>
        </div>
      ...
      {loadingTAK ? (
        ...
        <div className="flex flex-col items-center justify-center p-8 bg-rose-50 border border-rose-200 rounded-lg text-center">
          ...
          <p className="text-rose-700 mt-1">Terjadi kesalahan saat mengambil data TAK.</p>
          <button 
            onClick={() => {
              setTakError(false);
              setLoadingTAK(true);
                      ...
                      <div className="flex gap-2">
                        {request.result_path && (
                          <button
                            ...
                            title="Download TAK data"
                            onClick={() => handleCustomDownload(request.result_path, `TAK-${request.branch_id}-${request.start_date}-${request.end_date}.csv`)}
                          >
                        ...
                        {userRole === 'superadmin' && (
                          <button
                            onClick={() => {
                              ...
                              if (window.confirm(`Are you sure you want to delete this TAK request for branch ${request.branch_id}?`)) {
